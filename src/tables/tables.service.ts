...
export class TablesService {
  ...
  async startTable(tableId: string) {
    ...
    function generateRandomAI(TableArray: []) {
      let ias = [];

      for (let i = 0; i < 4 - TableArray.length; i++) {
        ias.push({
          id: i + 1,
          name: 'IA ' + (i + 1),
          balance: 1000,
  ...
  async actionTable(tableId: string, action: string) {
    if (!(action === 'fold' || action === 'call' || action === 'raise' || action === 'check') ) {
      throw new BadRequestException('Action invalide. Action possible : fold, call, raise, check.');
    }
